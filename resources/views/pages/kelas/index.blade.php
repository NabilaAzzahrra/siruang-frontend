<x-app-layout>
                                ...
                                <div class="p-4 rounded-xl">
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <input type="text" name="program_studi"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            placeholder="Masukan program studi disini ..." required />
                                    </div>
                                    <button type="submit"
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button type="button" data-id="{{ $d->id }}"
                                                            ...
                                                            data-kelas="{{ $d->kelas }}"
                                                            data-program_studi="{{ $d->program_studi }}"
                                                        ...
                                                        @else
                                                            <button
                                                                onclick="return dataDelete('{{ $d->id }}','{{ $d->kelas }}')"
    ...
    <script>
        $(document).ready(function() {
            $('#kelas-datatable').DataTable(); // Inisialisasi sederhana
        });
    </script>
    <script>
        const editSourceModal = (button) => {
            ...
            const program_studi = button.dataset.program_studi;
            let url = "{{ route('kelas.update', ':id') }}".replace(':id', id);
            console.log(url);
            let status = document.getElementById(modalTarget);
            document.getElementById('title_source').innerText = `Update ${kelas}`;
            document.getElementById('kelas').value = kelas;
            document.getElementById('program_studi').value = program_studi;
        ...
        const dataDelete = async (id, kelas) => {
            let tanya = confirm(`Apakah anda yakin untuk menghapus tahun akademik ${kelas} ?`);
            if (tanya) {
